...
parser.add_argument('--data_path', default='./data/clothing1M/images', type=str, help='path to dataset')
...
def Calculate_JSD(epoch,model1, model2):
    ...
    num_samples2 = args.num_batches*args.batch_size
    prob2 = torch.zeros(num_samples)
    ...
    for batch_idx, (inputs, targets, path,_) in enumerate(eval_loader):
        ...
        dist1 = JS_dist(out1, out2)
        prob2[int(batch_idx*batch_size):int((batch_idx+1)*batch_size)] = dist1
...
loader = dataloader.clothing_dataloader(root=args.data_path, batch_size=args.batch_size, warmup_batch_size = args.batch_size*2, num_workers=4, num_batches=args.num_batches)
print('| Building Net')
...
scheduler1 = optim.lr_scheduler.CosineAnnealingLR(optimizer1, 200, 1e-5)
scheduler2 = optim.lr_scheduler.CosineAnnealingLR(optimizer2, 200, 1e-5)
...
model_name_2 = 'clothing1m_net1.pth.tar'
...
for epoch in range(0, args.num_epochs+1):  
    ...
    val_loader = loader.run(0, 0,'val')

    if epoch>100:
        nb_repeat2 = 1  ## Change how many times we want to repeat on the same selection

    if epoch<warm_up:   

        train_loader = loader.run(0,0,'warmup')
        print('Warmup Net1')
        ...
        for i in range(nb_repeat):
            ...
            prob,prob1,paths1 = Calculate_JSD(epoch, net1, net2)                          ## Calculate the JSD distances 
            threshold = torch.mean(prob) 
            threshold1 = torch.mean(prob1)                                          ## Simply Take the average as the threshold
            print(threshold)
            print(threshold1)
            ...
            for i in range (prob.size()[0]):
                if prob[i] < threshold: 
                    if prob1[i] >= threshold1+threshold1*(1-threshold)*(1-(epoch-warm_up)/200) and prob1[i] >= threshold1:
                        prob[i] = 0.9
                        count += 1

            print(count)
            print(count2)
            SR = torch.sum(prob<threshold).item()/prob.size()[0] 
            print(SR)

            labeled_trainloader = loader.run(SR, 0, 'train', prob= prob, paths=paths1)
            ...
            with torch.no_grad():
                for batch_idx, (inputs, targets, _,index) in enumerate(eval_loader):
                    ...
                    pred_scores, pred_labels = knn_predict(feature_u, feature_bank, feature_labels, 14, knn_k)
                    ...
                    for i in range(batch_size):
                        if prob[index[i]] > threshold:
                            ...
                            if pred_labels[i] != targets[i]:
                                if prob1[index[i]] > threshold1*(1-(epoch-warm_up)/200)+threshold1*(1-threshold)*(1-(epoch-warm_up)/200):
                                    prob[index[i]] = 0.9
                                    count_label2 += 1
            ...
            for i in range(nb_repeat2):
                labeled_trainloader, unlabeled_trainloader = loader.run(SR, 1, 'train', prob= prob, paths=paths1)
                train(epoch, net1, net2, optimizer1, labeled_trainloader, unlabeled_trainloader)                        ## Train Net1
                acc1 = val(net1,val_loader,1)

        print('\n==== Net 1 evaluate next epoch training data loss ====') 

        eval_loader   = loader.run(SR,0,'eval_train')
        
        print('\nTrain Net2')

        for i in range(nb_repeat):
            prob,prob1,paths2 = Calculate_JSD(epoch, net1, net2)  
            ...
            labeled_trainloader = loader.run(SR, 0, 'train', prob= prob, paths=paths2)
            ...
            for i in range(nb_repeat2):
                labeled_trainloader, unlabeled_trainloader = loader.run(SR, 1, 'train', prob= prob, paths=paths2)
    ...
    test_loader = loader.run(0,0,'test')  
