...
class imagenet_dataset(Dataset):
    def __init__(self, root_dir, transform, num_class):
        self.root = os.path.join(root_dir, 'ILSVRC2012_img_val')
...
class webvision_dataset(Dataset): 
    def __init__(self, root_dir, transform, sample_ratio, epoch, mode, num_class, pred=[], probability=[], log=''): 
        self.root = root_dir
        self.transform = transform
        self.mode = mode  
            ...
            if self.mode == 'all':
                ...
                if self.mode == "labeled":
                    ...
                    for i in range(num_class):
                        ...
                        #sorted_indices  = np.argsort(-probability[self.class_ind[i]])
                        ##############################################################
                        # print(class_indices)
                        ...
                        try:
                            ...
                            #pred_idx[size_pred:size_pred+class_len] = np.array(class_indices)[sorted_indices[0:class_len].astype(int)].squeeze()
                            size_pred += class_len
                        except:
...
class webvision_dataloader():  
    ...
    def run(self,SR, epoch, mode, pred=[],prob=[]):
        if mode=='warmup':
            all_dataset = webvision_dataset(root_dir=self.root_dir, transform = self.transforms["warmup"], sample_ratio = SR, epoch=epoch,mode="all", num_class=self.num_class)                
            trainloader = DataLoader(
                dataset=all_dataset, 
                batch_size=96, #self.batch_size*2, #4
                shuffle=True,
            ...
            labeled_dataset = webvision_dataset(root_dir=self.root_dir, transform = self.transforms["labeled"],  sample_ratio = SR, epoch=epoch,mode="labeled",num_class=self.num_class,pred=pred,probability=prob,log=self.log)              
            ...
            if epoch != 0:
                unlabeled_dataset = webvision_dataset(root_dir=self.root_dir, transform = self.transforms["unlabeled"],  sample_ratio = SR,epoch=epoch, mode="unlabeled",num_class=self.num_class,pred=pred,log=self.log)                    
            ...
            test_dataset = webvision_dataset(root_dir=self.root_dir, transform = self.transforms["test"],  sample_ratio = SR, epoch=epoch,mode='test', num_class=self.num_class)      
            test_loader = DataLoader(
                dataset=test_dataset, 
                batch_size=480,#self.batch_size*10, #20
                shuffle=False,
                num_workers=self.num_workers,
                pin_memory=True)               
            return test_loader
            ...
            eval_dataset = webvision_dataset(root_dir=self.root_dir, transform = self.transforms["test"],  sample_ratio = SR, epoch=epoch,mode='all', num_class=self.num_class)      
